...
// ------------------------------------------------------------------------
public class AsteroidsParserMyListener<E, S, T> implements
		AsteroidsParserListener {
	...
	private S StatementOfAssign(TerminalNode identifier,
			AsteroidsParserParser.ExprContext expr) {
		if (expr == null)
			return null;
	...
	private E ExpressionOfNamedConst(
			AsteroidsParserParser.NamedconstContext naconst) {
		if (naconst == null)
			return null;
	...
	private E ExpressionOfBinop(AsteroidsParserParser.BinopContext op,
			AsteroidsParserParser.ExprContext e1,
			AsteroidsParserParser.ExprContext e2) {
		if (op == null || e1 == null)
			return null;
	...
	private E ExpressionOfUnop(AsteroidsParserParser.UnopContext op) {
		...
		if (op.GETVY() != null) {
			return (factory.createGetVY(line, column,
					ExpressionOfExpr(op.expr())));
		}
		if (op.GETVX() != null) {
			return (factory.createGetVX(line, column,
					ExpressionOfExpr(op.expr())));
		}
		if (op.NOT() != null) {
			return (factory
	...
	private E ExpressionOfExpr(AsteroidsParserParser.ExprContext expr) {
		...
		case 0: {
			...
			if (expr.IDENTIFIER() != null) {
				return (factory.createVariable(line, column, expr.IDENTIFIER()
						.getText()));
			}
			if (expr.unop() != null) {
